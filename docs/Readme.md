...
All load balancer providers support graceful draining with provider-specific implementations:
...
- **Dummy**: Logs all drain operations for testing purposes
...
- **Immediate removal (0s)**: Very short-lived connections or applications that handle reconnection gracefully
...
spec:
  ...
  drain:                  # Optional graceful connection draining configuration (optional)
    ...
    timeoutSeconds: 30    # Time to wait before deleting members (default: 30, min: 0, max: 3600) (optional)
